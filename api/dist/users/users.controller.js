...
let UsersController = class UsersController {
    ...
    update(id, updateUserDto, req) {
        if (req.user.userId !== id) {
            throw new common_1.ForbiddenException('Você só pode atualizar o próprio perfil');
        }
        return this.usersService.update(id, updateUserDto);
    }
    async remove(id, req, res) {
        if (req.user.userId !== id) {
            throw new common_1.ForbiddenException('Você só pode excluir a própria conta');
        }
